...
export default function ChatHeader({ conversation, currentUserId }: Props) {
  ...
  const avatarSrc = conversation.isGroup
    ? conversation.groupImage ||
      generateAvatarUrl(conversation.groupName || "Group")
    : otherUser?.image || "";

  const isOnline =
    !conversation.isGroup && otherUser && onlineUsers.includes(otherUser._id);
  const typing = typingUsers[conversation._id];
  const isTyping = typing && typing.length > 0;

  let status = "";
  if (isTyping)
    status = `${typing.map((t) => t.userName).join(", ")} ${typing.length === 1 ? "is" : "are"} typing...`;
    ...
    >
      <div className="flex items-center gap-2 md:gap-3 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <p
            className={`text-xs truncate ${isTyping ? "text-primary italic" : isOnline ? "text-green-500" : "text-on-surface-variant"}`}
          >
            {status}
          </p>
        </div>
