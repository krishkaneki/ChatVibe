...
export default function ProfileClient({ user }: { user: UserData }) {
  ...
  const imageDisplaySrc = image
    ? `${image}${image.includes("?") ? "&" : "?"}v=${imageVersion || "0"}`
    : "";
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div className="relative w-fit" {...getRootProps()}>
                ...
                {imageDisplaySrc ? (
                  ...
                  <div
                    className="w-20 h-20 md:w-[88px] md:h-[88px] rounded-2xl border-4 border-surface-container shadow-lg flex items-center justify-center text-white text-3xl font-bold"
                    style={{ background: "var(--signature-gradient)" }}
                  >
                    {initials}
                  </div>
                ...
                <div
                  className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full flex items-center justify-center cursor-pointer shadow"
                  style={{ background: "var(--signature-gradient)" }}
                >
